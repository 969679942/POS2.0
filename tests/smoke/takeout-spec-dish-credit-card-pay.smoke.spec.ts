...
test.describe('【To Go-购买规格菜-点单页Credit Card付款】', () => {
  test(
    '应能完成 To Go 规格菜点单与点单页信用卡读卡支付，点单页汇总与支付成功页 Balance due 一致',
    {
      tag: ['@smoke'],
      annotation: [
        {
    ...
    async ({ homePage, licenseSelectionPage, employeeLoginPage }) => {
      ...
      const loggedInHomePage = await test.step('前置：员工口令登录进入 POS 主页', async () => {
        return await enterWithEmployeePassword(
          employeeLoginPage,
          homePage,
          takeOutSpecDishCashSmokeTestData.employeePassword,
        );
      });

      await loggedInHomePage.expectPrimaryFunctionCardsVisible();

      const orderDishesPage = await test.step('步骤1：首页进入 To Go 点单页并搜索规格菜', async () => {
