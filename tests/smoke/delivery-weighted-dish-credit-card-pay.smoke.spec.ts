...
test.describe('【Delivery-购买称重菜-点单页Credit Card付款】', () => {
  test(
    '应能完成 Delivery 称重菜点单与点单页信用卡读卡支付，点单页汇总与支付成功页 Balance due 一致',
    {
      tag: ['@smoke'],
      annotation: [
        {
    ...
    async ({ homePage, licenseSelectionPage, employeeLoginPage }) => {
      ...
      const orderDishesPage = await test.step('步骤1：首页经 Delivery 填单进入点单页并搜索称重菜', async () => {
