...
test.describe('【Admin-菜单-创建开价菜】', () => {
  test(
    'Admin 菜单管理：在首个 Category 中新建开价菜并可在 To Go 搜索到',
    {
      tag: ['@smoke'],
    },
    async ({ homePage, licenseSelectionPage, employeeLoginPage }) => {
      ...
      const item = itemFlow.buildItemScenario('openPrice');
      ...
      await test.step('步骤8：CREATE NEW → 新建开价菜，勾选 Open Price Item 并保存', async () => {
      ...
      const orderDishesPage = await test.step('步骤9：首页进入 To Go 点单页并搜索新建开价菜', async () => {
      ...
      await test.step('断言2：To Go 点单页可搜索到新建开价菜', async () => {
