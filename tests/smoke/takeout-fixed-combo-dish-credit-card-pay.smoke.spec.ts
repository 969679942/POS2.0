...
import { takeOutFixedComboDishCashSmokeTestData } from '../../test-data/takeout-fixed-combo-dish-smoke';
...
test.describe('【To Go-购买固定套餐-点单页Credit Card付款】', () => {
  test(
    '应能完成 To Go 固定套餐点单与点单页信用卡读卡支付，点单页汇总与支付成功页 Balance due 一致',
    {
      tag: ['@smoke'],
      annotation: [
        {
    ...
    async ({ homePage, licenseSelectionPage, employeeLoginPage }) => {
      ...
      const loggedInHomePage = await test.step('前置：员工口令登录进入 POS 主页', async () => {
        return await enterWithEmployeePassword(
          employeeLoginPage,
          homePage,
          takeOutFixedComboDishCashSmokeTestData.employeePassword,
        );
      });

      await loggedInHomePage.expectPrimaryFunctionCardsVisible();

      const orderDishesPage = await test.step('步骤1：首页进入 To Go 点单页并搜索固定套餐', async () => {
        const odp = await startToGoOrder(loggedInHomePage);
        await odp.openDishSearchPanel();
        await odp.applyDishSearchKeyword(takeOutFixedComboDishCashSmokeTestData.fixedComboDishSearchKeyword);
        await odp.expectDishSearchResultVisible(
          takeOutFixedComboDishCashSmokeTestData.fixedComboDishSearchKeyword,
        );
        return odp;
      });

      await test.step('步骤2：点选固定套餐并按产品默认或自动补齐必选项', async () => {
        await addComboDish(
          orderDishesPage,
          takeOutFixedComboDishCashSmokeTestData.fixedComboDishSearchKeyword,
          takeOutFixedComboDishCashSmokeTestData.comboSelections,
          1,
        );
      });
